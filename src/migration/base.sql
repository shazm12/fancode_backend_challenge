...
-- Create indexes for table tours
create index idx_tours_name ON mydb.tours (name);
create index idx_tours_id ON mydb.tours (id);
...
-- Task 3: Create News Entity/Model
...
insert ignore into mydb.news (title, description, tourId, matchId) values ("Will CSK Win?", "Will CSK Win?", null, 2);
insert ignore into mydb.news (title, description, tourId, matchId) values ("DLF Invests heavily on IPL", "DLF Invests heavily on IPL", 1, null);